...
redisVersion=5.0.14
downurl="https://gushen-ecp.obs.cn-south-1.myhuaweicloud.com/redis/redis-5.0.14.tar.gz"
port=6379
params=$*
...
--packagedir       选填参数,安装包目录,默认/sie/packagedir
--logdir           选填参数,日志目录,默认/sie/redis/logs
--datadir          选填参数,数据目录,默认/sie/redis/data
--backdir          选填参数,备份目录,默认/sie/backup/redisback
--password         *访问密码
...
then
    ...
    echo "备份目录: backdir:${backdir}/redis目录已存在,请检查是否有数据,程序将自动退出!"
    exit 9
fi
# 下载
if [[ ! -f "${packagedir}/redis-${redisVersion}.tar.gz" ]]; then
  echo "安装包不存在,下载中......"
  mkdir ${packagedir}
...
tar -xf ${packagedir}/redis-${redisVersion}.tar.gz  -C ${rootdir}
mkdir -p ${logdir}
cp ${rootdir}/redis-${redisVersion}/redis.conf ${rootdir}/redis-${redisVersion}/redis.conf.bak

cat>${rootdir}/redis-${redisVersion}/redis.conf <<EOF
bind $host
protected-mode no
port $port
tcp-backlog 511
...
daemonize yes
supervised no
pidfile ${rootdir}/redis-${redisVersion}/redis_6379.pid
loglevel notice
...
chown -R ${RunUser}:${RunGroup} ${backdir}
chown -R ${RunUser}:${RunGroup} ${packagedir}
...
Type=forking
PIDFile=${rootdir}/redis-${redisVersion}/redis_6379.pid
ExecStart=${rootdir}/redis-${redisVersion}/src/redis-server  ${rootdir}/redis-${redisVersion}/redis.conf
...
RedisPID=$(systemctl status redis | grep "Main PID" | awk -F ":" '{print$2}' | awk '{print$1}')
psWC=$(ps -ef | grep "$RedisPID" | wc -l)

if [ "${psWC}" -gt 1 ];then
    echo '======================================================='
    echo '======================================================='
    echo 'Redis5.0.14 安装成功！！！'
    echo "Redis5.0.14服务所安装地址为: ${host}:${port}"
    echo "请导入SQL数据......"
    echo '======================================================='
    echo '======================================================='
...
cat redis.sql | ${rootdir}/redis-${redisVersion}/src/redis-cli -a ${password} -p $port
...
echo "backdir备份目录为: ${backdir}/redis"
